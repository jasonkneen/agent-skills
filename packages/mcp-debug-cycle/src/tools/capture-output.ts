...
export const captureOutputTool: Tool = {
  name: "debug_capture_output",
  description:
    "Run a command and capture its output (stdout, stderr) for analysis. Saves output to ~/.claude/debug-captures/",
  inputSchema: {
    type: "object",
    properties: {
      command: {
...
export async function captureOutput(args: CaptureOutputArgs): Promise<ToolResult> {
  const {
    command,
    args: cmdArgs = [],
    cwd,
