...
export const searchIssuesTool: Tool = {
  ...
  description:
    "Search GitHub issues and PRs using full-text search with optional filters",
  inputSchema: {
    type: "object",
    properties: {
      owner: {
...
export async function searchIssues(args: SearchIssuesArgs): Promise<ToolResult> {
  const {
    owner,
    repo,
    query,
  ...
  const result = await gh<GitHubSearchItem[]>(ghArgs)
